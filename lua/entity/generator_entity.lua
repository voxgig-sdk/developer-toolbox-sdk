-- DeveloperToolbox SDK Generator entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local GeneratorEntity = {}
GeneratorEntity.__index = GeneratorEntity


function GeneratorEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GeneratorEntity)
...
function GeneratorEntity:get_name()
  return self._name
end


function GeneratorEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GeneratorEntity.new(self._client, opts)
end
...
function GeneratorEntity:mark_deleted()
  self._deleted = true
end


function GeneratorEntity:deleted()
  return true == self._deleted
end


function GeneratorEntity:data_set(args)
  if args ~= nil then
...
function GeneratorEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GeneratorEntity:match_set(args)
  if args ~= nil then
...
function GeneratorEntity:match_get()
...
function GeneratorEntity:stream(action, args, callopts)
...
function GeneratorEntity:_run_op(ctx, post_done)
  local utility = self._utility
