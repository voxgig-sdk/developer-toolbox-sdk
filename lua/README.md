# DeveloperToolbox Lua SDK



The Lua SDK for the DeveloperToolbox API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Generator()` — each with the same small set of operations (`list`, `load`, `create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```
lua/
├── developer-toolbox_sdk.lua    -- Main SDK module
...
The main module (`developer-toolbox_sdk`) exports the SDK constructor
