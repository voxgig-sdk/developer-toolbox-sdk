...
The Lua SDK for the DeveloperToolbox API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Generator(nil):list(nil, nil)
...
```lua
local result, err = client:Generator(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
```lua
-- Create
local created, _ = client:Generator(nil):create({ name = "Example" }, nil)
...
local result, err = client:DeveloperToolbox(nil):load(
  { id = "test01" }, nil
)
