...
require("developer-toolbox_types")
...
local DeveloperToolboxSDK = {}
DeveloperToolboxSDK.__index = DeveloperToolboxSDK
...
DeveloperToolboxSDK._make_feature = _make_feature


function DeveloperToolboxSDK.new(options)
  local self = setmetatable({}, DeveloperToolboxSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function DeveloperToolboxSDK:options_map()
...
function DeveloperToolboxSDK:get_utility()
  return Utility.copy(self._utility)
end


function DeveloperToolboxSDK:get_root_ctx()
  return self._rootctx
end


function DeveloperToolboxSDK:prepare(fetchargs)
...
function DeveloperToolboxSDK:direct(fetchargs)
...
function DeveloperToolboxSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function DeveloperToolboxSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "DeveloperToolboxSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function DeveloperToolboxSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function DeveloperToolboxSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "DeveloperToolboxSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function DeveloperToolboxSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = DeveloperToolboxSDK.new(sdkopts)
  sdk.mode = "test"
