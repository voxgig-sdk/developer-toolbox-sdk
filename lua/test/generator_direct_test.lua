...
describe("GeneratorDirect", function()
  it("should direct-list-generator", function()
    local setup = generator_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-generator", setup.live and "live" or "unit")
  ...
  it("should direct-load-generator", function()
    local setup = generator_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-generator", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
