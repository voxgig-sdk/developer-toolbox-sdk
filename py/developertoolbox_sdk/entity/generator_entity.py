...
from developertoolbox_sdk.developertoolbox_types import (
    Generator,
    GeneratorLoadMatch,
    GeneratorListMatch,
    GeneratorCreateData,
...
class GeneratorEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return GeneratorEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Generator:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Generator().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Generator]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Generator().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
    ...
    def create(self, reqdata: GeneratorCreateData, ctrl=None) -> Generator:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
