...
class TestUrlToolEntity:

    def test_should_create_instance(self):
        testsdk = DeveloperToolboxSDK.test(None, None)
        ent = testsdk.UrlTool(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _url_tool_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "url_tool." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set DEVELOPER_TOOLBOX_TEST_URL_TOOL_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        url_tool_ref01_ent = client.UrlTool(None)
        url_tool_ref01_data = helpers.to_map(vs.getprop(
            vs.getpath(setup["data"], "new.url_tool"), "url_tool_ref01"))

        url_tool_ref01_data = helpers.to_map(runner.entity_data(url_tool_ref01_ent.create(url_tool_ref01_data, None)))
        assert url_tool_ref01_data is not None



def _url_tool_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/url_tool/UrlToolTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "DEVELOPER_TOOLBOX_TEST_URL_TOOL_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("DEVELOPER_TOOLBOX_TEST_URL_TOOL_ENTID"))
