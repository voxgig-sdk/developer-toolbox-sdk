...
local function make_error_util(ctx, err)
  ...
  local msg = "DeveloperToolboxSDK: " .. opname .. ": " .. errmsg
  ...
  local sdk_err = DeveloperToolboxError.new("", msg, ctx)
  sdk_err.result = ctx.utility.clean(ctx, result)
  sdk_err.spec = ctx.utility.clean(ctx, spec)

  if type(err) == "table" and getmetatable(err) == DeveloperToolboxError then
