...
// DeveloperToolbox SDK pipeline test
//
...
{
    private static function utility(): DeveloperToolboxUtility
    {
        return new DeveloperToolboxUtility();
    }
    ...
    private static function ctx(array $over = []): DeveloperToolboxContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new DeveloperToolboxContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new DeveloperToolboxOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof DeveloperToolboxError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new DeveloperToolboxOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = DeveloperToolboxMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new DeveloperToolboxOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = DeveloperToolboxMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new DeveloperToolboxOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = DeveloperToolboxMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DeveloperToolboxSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = DeveloperToolboxMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new DeveloperToolboxResponse([]),
            'result' => new DeveloperToolboxResult([]),
        ]);
        [, $err] = DeveloperToolboxMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new DeveloperToolboxSpec([]),
            'response' => null,
            'result' => new DeveloperToolboxResult([]),
        ]);
        [, $err] = DeveloperToolboxMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new DeveloperToolboxSpec(['step' => 's']),
            'response' => new DeveloperToolboxResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new DeveloperToolboxResult([]),
        ]);
        [, $err] = DeveloperToolboxMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new DeveloperToolboxSpec(['step' => 's']),
            'response' => new DeveloperToolboxResponse(self::resp(200, ['v' => 1])),
            'result' => new DeveloperToolboxResult([]),
        ]);
        [, $err] = DeveloperToolboxMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new DeveloperToolboxSpec(['step' => 's']),
            'response' => new DeveloperToolboxResponse(self::resp(200, ['v' => 2])),
            'result' => new DeveloperToolboxResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        DeveloperToolboxMakeResponse::call($ctx);
    ...
    {
        $preset = new DeveloperToolboxResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new DeveloperToolboxSpec([]),
            'response' => new DeveloperToolboxResponse([]),
            'result' => new DeveloperToolboxResult([]),
        ...
        [$got, $err] = DeveloperToolboxMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new DeveloperToolboxResult([])]);
        [, $err] = DeveloperToolboxMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new DeveloperToolboxSpec([]), 'result' => null]);
        [, $err] = DeveloperToolboxMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DeveloperToolboxSpec(['step' => 's']),
            'result' => new DeveloperToolboxResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new DeveloperToolboxOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DeveloperToolboxMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DeveloperToolboxSpec(['step' => 's']),
            'result' => new DeveloperToolboxResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new DeveloperToolboxOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DeveloperToolboxMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DeveloperToolboxResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new DeveloperToolboxSpec([]),
            'result' => new DeveloperToolboxResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = DeveloperToolboxMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DeveloperToolboxMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new DeveloperToolboxError('boom', 'boom');
        $utility->fetcher = function (DeveloperToolboxContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DeveloperToolboxSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = DeveloperToolboxMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (DeveloperToolboxContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = DeveloperToolboxMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(DeveloperToolboxResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        DeveloperToolboxMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (DeveloperToolboxContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DeveloperToolboxSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = DeveloperToolboxMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new DeveloperToolboxResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new DeveloperToolboxSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = DeveloperToolboxMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = DeveloperToolboxMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = DeveloperToolboxMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new DeveloperToolboxResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, DeveloperToolboxDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new DeveloperToolboxResult(['ok' => false])]);
        $this->expectException(DeveloperToolboxError::class);
        DeveloperToolboxDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new DeveloperToolboxResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', DeveloperToolboxMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        DeveloperToolboxMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new DeveloperToolboxBaseFeature();
        $b = new DeveloperToolboxBaseFeature();
        DeveloperToolboxFeatureAdd::call($ctx, $a);
        DeveloperToolboxFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): DeveloperToolboxBaseFeature
    {
        $f = new DeveloperToolboxBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        DeveloperToolboxFeatureAdd::call($ctx, self::named_feature('a'));
        DeveloperToolboxFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        DeveloperToolboxFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        DeveloperToolboxFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        DeveloperToolboxFeatureAdd::call($ctx, $replace);
        ...
        DeveloperToolboxFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): DeveloperToolboxContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new DeveloperToolboxSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DeveloperToolboxPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        DeveloperToolboxPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new DeveloperToolboxResponse(['headers' => null]),
            'result' => new DeveloperToolboxResult([]),
        ]);
        DeveloperToolboxResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        DeveloperToolboxResultBody::call($ctx);
