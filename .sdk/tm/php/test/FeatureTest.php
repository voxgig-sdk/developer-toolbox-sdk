...
// DeveloperToolbox SDK feature test
//
...
class FtCtrl extends DeveloperToolboxControl
{
...
class FtClient
{
    ...
    private DeveloperToolboxUtility $utility;

    public function __construct(DeveloperToolboxUtility $utility, array $options)
    {
    ...
    public function get_utility(): DeveloperToolboxUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (DeveloperToolboxContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public DeveloperToolboxContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (DeveloperToolboxContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = DeveloperToolboxConfig::make_feature($name);
    ...
    private static function buildUrl(DeveloperToolboxSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof DeveloperToolboxError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new DeveloperToolboxResponse($fetched);
        }
    ...
    private function fail(DeveloperToolboxContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (DeveloperToolboxContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
