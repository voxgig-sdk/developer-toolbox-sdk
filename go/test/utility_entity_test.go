...
func TestUtilityEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		utilityRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "utility"}), "utility_ref01"))

		utilityRef01DataResult, err := utilityRef01Ent.Create(utilityRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		utilityRef01Data = core.ToMapAny(entityData(utilityRef01DataResult))
...
func utilityBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("DEVELOPER_TOOLBOX_TEST_UTILITY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["DEVELOPER_TOOLBOX_TEST_UTILITY_ENTID"])
