...
import (
	...
	omni "github.com/voxgig-sdk/developer-toolbox-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.DeveloperToolboxError); is {
				out := map[string]any{
					"name":    "DeveloperToolboxError",
...
func omniCtx(arg any, client *sdk.DeveloperToolboxSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.DeveloperToolboxSDK); is {
			client = live
