...
func TestGeneratorEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Generator(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Generator(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		generatorRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "generator"}), "generator_ref01"))

		generatorRef01DataResult, err := generatorRef01Ent.Create(generatorRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		generatorRef01Data = core.ToMapAny(entityData(generatorRef01DataResult))
		...
		_, generatorRef01ListOk := generatorRef01ListResult.([]any)
...
func generatorBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("DEVELOPER_TOOLBOX_TEST_GENERATOR_ENTID")
	...
	idmapResolved := core.ToMapAny(env["DEVELOPER_TOOLBOX_TEST_GENERATOR_ENTID"])
