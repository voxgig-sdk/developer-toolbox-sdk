...
[MCP](https://modelcontextprotocol.io) server exposing the DeveloperToolbox SDK as
two agent tools — `developer-toolbox_list` and `developer-toolbox_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/developer-toolbox-mcp)
make build

# 2. Provide credentials via the environment
export DEVELOPER_TOOLBOX_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user developer-toolbox \
  -- /absolute/path/to/developer-toolbox-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./developer-toolbox-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// developer-toolbox_list: first page of records
{ "entity": "generator" }
{ "entity": "generator", "query": { } }

// developer-toolbox_load: one record by id
{ "entity": "generator", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/developer-toolbox-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user developer-toolbox \
     -- "$PWD"/dist/*/developer-toolbox-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `developer-toolbox_list` and `developer-toolbox_load` tools now appear
   in new sessions. Ask the agent to *"list generator using developer-toolbox"*
   and it calls `developer-toolbox_list` with `{"entity":"generator"}`.
...
```sh
export DEVELOPER_TOOLBOX_APIKEY=sk_live_xxx            # API key
export DEVELOPER_TOOLBOX_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `developer-toolbox_list` tool
...
### Call the `developer-toolbox_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `developer-toolbox_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `developer-toolbox_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `DEVELOPER_TOOLBOX_APIKEY` | API key sent with every request. |
| `DEVELOPER_TOOLBOX_BASE` | Optional override of the API base URL. |
...
```sh
./developer-toolbox-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"developer-toolbox_load","arguments":{"entity":"generator","query":{"id":1}}}}'
```
