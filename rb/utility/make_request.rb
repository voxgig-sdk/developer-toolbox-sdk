...
module DeveloperToolboxUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = DeveloperToolboxResponse.new({})
    ...
    if fetch_err
      ...
      response = DeveloperToolboxResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = DeveloperToolboxResponse.new(fetched)
    else
