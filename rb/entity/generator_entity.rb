...
class GeneratorEntity
  ...
  def make
    opts = @_entopts.dup
    GeneratorEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.Generator.load works with no args).
  ...
  #
  # @param reqmatch [GeneratorListMatch, Hash, nil] match filter (any subset of
  #   Generator fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
