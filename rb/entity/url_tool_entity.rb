...
class UrlToolEntity
  ...
  def make
    opts = @_entopts.dup
    UrlToolEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = DeveloperToolboxHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [UrlTool, Hash] the current UrlTool data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = DeveloperToolboxHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of UrlTool fields)
  def match_get
  ...
  #
  # @param reqdata [UrlToolCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [UrlTool, Hash] the created UrlTool; raises DeveloperToolboxError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = DeveloperToolboxHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
