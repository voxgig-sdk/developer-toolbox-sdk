...
require_relative 'DeveloperToolbox_types'
...
class DeveloperToolboxSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = DeveloperToolboxHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = DeveloperToolboxHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, DeveloperToolboxFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    DeveloperToolboxUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = DeveloperToolboxHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = DeveloperToolboxHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = DeveloperToolboxHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => DeveloperToolboxError.new(
        "#{op}_allow",
        "DeveloperToolboxSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue DeveloperToolboxError => err
    ...
    if fetched.is_a?(Hash)
      status = DeveloperToolboxHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = DeveloperToolboxError.new(
        "graphql_error", "DeveloperToolboxSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = DeveloperToolboxSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
