...
class UtilityEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set DEVELOPER_TOOLBOX_TEST_UTILITY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    utility_ref01_data = Helpers.to_map(utility_ref01_data_result.respond_to?(:data_get) ? utility_ref01_data_result.data_get : utility_ref01_data_result)
...
def utility_basic_setup(extra)
  ...
  entid_env_raw = ENV["DEVELOPER_TOOLBOX_TEST_UTILITY_ENTID"]
