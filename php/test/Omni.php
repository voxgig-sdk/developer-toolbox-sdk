...
//    only - a typed DeveloperToolboxContext there would make every ctx
...
final class DeveloperToolboxOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new DeveloperToolboxContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new DeveloperToolboxSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new DeveloperToolboxResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new DeveloperToolboxError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new DeveloperToolboxResponse($clean['response']);
    ...
    public static function err_from_map($m): ?DeveloperToolboxError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new DeveloperToolboxError(is_string($code) ? $code : '', $msg);
    }
