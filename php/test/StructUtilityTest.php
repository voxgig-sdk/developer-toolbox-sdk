...
// DeveloperToolbox SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = DeveloperToolboxOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(DeveloperToolboxOmni::structfix($now, Struct::undef()))
                        !== json_encode(DeveloperToolboxOmni::structfix($val, Struct::undef()))) {
