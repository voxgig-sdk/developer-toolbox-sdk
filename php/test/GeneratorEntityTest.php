...
class GeneratorEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = DeveloperToolboxSDK::test($seed, null);
        $seen = iterator_to_array($base->Generator(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = DeveloperToolboxConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = DeveloperToolboxSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->Generator(null)->stream("list", null, null) as $item) {
