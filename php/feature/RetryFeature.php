...
// DeveloperToolbox SDK retry feature
...
class DeveloperToolboxRetryFeature extends DeveloperToolboxBaseFeature
{
    ...
    public function init(DeveloperToolboxContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (DeveloperToolboxContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(DeveloperToolboxContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(DeveloperToolboxContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
