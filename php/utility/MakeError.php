...
class DeveloperToolboxMakeError
{
    public static function call(?DeveloperToolboxContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new DeveloperToolboxContext([], null);
        }
        $op = $ctx->op ?? new DeveloperToolboxOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new DeveloperToolboxResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof DeveloperToolboxError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new DeveloperToolboxError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof DeveloperToolboxError) {
