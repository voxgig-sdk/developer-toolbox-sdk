...
// DeveloperToolbox SDK Utility entity
...
class UtilityEntity
{
    ...
    {
        if ($args) {
            $this->_data = DeveloperToolboxHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = DeveloperToolboxHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *
     * @param UtilityCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed UtilityCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Utility|array The created Utility as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = DeveloperToolboxHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
