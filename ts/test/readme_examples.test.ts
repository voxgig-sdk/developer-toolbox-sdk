...
//     (\`new DeveloperToolboxSDK(...)\` / \`DeveloperToolboxSDK.test(...)\` become
//     \`DeveloperToolboxSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'DeveloperToolboxSDK'
...
const TEST_SEED = {"entity":{"generator":{"test01":{"id":"test01"}},"url_tool":{"test01":{"id":"test01"}},"utility":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new DeveloperToolboxSDK(...)\` and any
// \`DeveloperToolboxSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+DeveloperToolboxSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/DeveloperToolboxSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
