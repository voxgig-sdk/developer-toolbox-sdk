...
describe('GeneratorDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new DeveloperToolboxSDK({
  ...
  test('direct-load-generator', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-generator', setup.live)) return
  ...
  test('direct-list-generator', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-generator', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['DEVELOPER_TOOLBOX_TEST_GENERATOR_ENTID']
