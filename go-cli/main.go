// Command DeveloperToolbox-cli is a CLI + REPL for the DeveloperToolbox SDK,
...
const prompt = "developer-toolbox"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "generator url_tool utility"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      DEVELOPER_TOOLBOX_APIKEY (api key), DEVELOPER_TOOLBOX_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: DEVELOPER_TOOLBOX_APIKEY carries the API key and
	// DEVELOPER_TOOLBOX_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("DEVELOPER_TOOLBOX_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("DEVELOPER_TOOLBOX_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewDeveloperToolboxSDK(opts)
