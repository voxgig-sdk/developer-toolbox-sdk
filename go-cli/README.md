...
boru-driven command-line client **and** interactive REPL for the DeveloperToolbox
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/developer-toolbox-cli)
make build

# 2. See usage (words, entities, env vars)
./developer-toolbox-cli --help
...
./developer-toolbox-cli load 1 generator            # {id:1} shorthand
./developer-toolbox-cli load '{id:1}' generator       # explicit match map
./developer-toolbox-cli list url_tool

# 5. Override the API base URL for a single call
DEVELOPER_TOOLBOX_BASE=https://api.example.com ./developer-toolbox-cli list generator

# 6. No arguments -> interactive REPL
./developer-toolbox-cli
developer-toolbox> list generator
developer-toolbox> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/developer-toolbox-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/developer-toolbox-cli list generator
   ```
...
```sh
./developer-toolbox-cli load 1 generator          # scalar shorthand for {id:1}
./developer-toolbox-cli load '{id:1}' generator     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `developer-toolbox>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `generator`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/developer-toolbox-cli`. |
...
(`./developer-toolbox-cli <expr>`) and interactively (the REPL), and expressions compose the
